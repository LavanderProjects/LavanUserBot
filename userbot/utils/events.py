...
async def get_user_from_event(
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.sender_id is None:
                if not noedits:
                    await edit_delete(
                        manevent, "**HATA: O isimsiz bir yönetici!**", 60
                    )
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    manevent,
                    "**Lütfen Mesajı Yanıtlayın veya Kullanıcı Kimliği / Kullanıcı Adı Girin!**",
                    60,
                )
            return None, None
    except Exception as e:
    ...
    if not noedits:
        await edit_delete(
            manevent,
            "**Lütfen mesajı yanıtlayın veya kullanıcı kimliği/kullanıcı adı sağlayın!**",
            60,
        )
    return None, None
