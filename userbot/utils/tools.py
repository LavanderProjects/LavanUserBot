...
async def extract_time(man, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        ...
        if not time_num.isdigit():
            await man.edit("Belirtilen süre geçersiz.")
            return None
        if unit == "s":
            ...
            await man.edit(
                f"**Girilen zaman türü geçersiz. Lütfen** s, m , h , d veya w girin, ancak: `{time_val[-1]}`"
            )
            return None
        return bantime
    await man.edit(
...
async def edit_or_reply(
    ...
    if aslink or deflink:
        linktext = linktext or "**Mesaj Çok Uzun**"
        response = await paste_message(text, pastetype="s")
        text = f"{linktext} [Buraya bak]({response})"
        if not event.out and event.sender_id in SUDO_USERS:
            if reply_to:
...
def post_to_telegraph(title, html_format_content):
    ...
    auth_name = "LavanUserBot"
    auth_url = "https://github.com/LavanderProjects/LavanUserBot"
