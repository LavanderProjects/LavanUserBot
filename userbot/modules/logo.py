...
async def logo_gen(event):
    ...
    if os.path.exists(fayladi):
        await event.client.send_file(
            event.chat_id,
            file=fayladi,
            caption=f"[LavanUserBot](https://t.me/Lavanuserbot) **{DEFAULT_NAME}** için logo hazırladı.",
            force_document=True,
        )
        os.remove(fayladi)
        await event.delete()
...
Help = CmdHelp('logo')
Help.add_command('logo', '<yazı>', '@LavanUserBot sizin için logo hazırlar.')
Help.add_info('@LavanUserBot - @ByMisakiMey ')
Help.add() 
