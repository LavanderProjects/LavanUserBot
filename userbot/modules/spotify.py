    ...
    while SPOTIFYCHECK:
        try:
            ...
            if song != oldsong and artist != oldartist:
                ...
                spobio = "@LavanUserBot 🎧: " + artist + " - " + song
                try:
...
def get_spotify_info(TIME=5):
    try:
        ...
        if len(item['album']['images']) > 0:
            ...
            if path.exists("@LavanUserBot-Spotify.jpg"):
                remove("@LavanUserBot-Spotify.jpg")          
            try:
                r = get(str(item['album']['images'][0]['url']))
                with open("@LavanUserBot-Spotify.jpg", 'wb') as f:
                    f.write(r.content)    

                with open('LavanUserBot-Spotify.jpg', 'rb') as f:
