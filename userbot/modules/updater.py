...
from userbot import CMD_HELP, HEROKU_APIKEY, HEROKU_APPNAME, UPSTREAM_REPO_URL, ASISTAN, MYID, AUTO_UPDATE, LAVAN_VERSION, upVer, EMERGENCY
...
async def upstream(ups):
    ...
    if conf != "now" and not force_update:
        LAVANVer = int(LAVAN_VERSION.split(".")[1])
        if LAVANVer < upVer:
          await ups.edit(f"**Lütfen Lavan yöneticileri izin vermeden güncelleme yapmaya çalışma\n Botun bozulabilir\n Güncelleme kanalım :** @LAVANUserBot")
          return
...
async def upstream(ups):
    LAVANVer = int(LAVAN_VERSION.split(".")[1])
    if LAVANVer < upVer:
     await ups.edit(f"**Lütfen Lavan yöneticileri izin vermeden güncelleme yapmaya çalişma\n Botun bozulabilir\n Güncelleme kanali :** @LAVANUserBot") #CR vERMEYEN NE OLSUN - ByMisakiMey
     return
    await ups.edit(LANG['DETECTING'])
    conf = ups.pattern_match.group(1)
    off_repo = EMERGENCY
