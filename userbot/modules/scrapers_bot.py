...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./LavanSticker.png"):
                os.remove("./LavanSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("LavanSticker.png", "png")
            Resim = "LavanSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' LavanSticker.png")
            os.remove(sticker)
            Resim = "LavanSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} LavanThumb.jpg")
            os.remove(Resim)
            Resim = 'LavanThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./Lavanmeme.png"):
            os.remove("./Lavanmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./Lavanmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
