...
async def pins(event):
  ...
  #  plugins = await event.client.get_messages('@Lavanplugin', limit=None, search=dosyaAdi, filter=InputMessagesFilterDocument)

  #  if len(plugins) == 0:
   #     await event.edit('🍕 `Pizzamı yemeye devam edeceğim. Bu bir Lavan Plugini değil!`')
 #       return
...
async def psend(event):
    ...
    if os.path.isfile(f"./userbot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./userbot/modules/{modul}.py", caption=LANG['Lavan_PLUGIN_CAPTION'])
        await event.delete()
    else:
