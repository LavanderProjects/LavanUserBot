...
CV_MSG = [
    "**{DEFAULT_NAME}** `Fazla Bi Bilgi Ayarlamamış Ama Şunu Biliyorum Kendisi Baya Zevkli Birisi Çünkü lavan Userbot Kullanıyor.` 😁",
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # lavanPY
            lavanpy = re.search('\"\"\"lavanPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not lavanpy == None:
