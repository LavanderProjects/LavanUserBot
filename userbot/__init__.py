...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        level=DEBUG,
        format="[%(asctime)s - %(levelname)s] - @LavanUserBot : %(message)s",
        datefmt='%d-%b-%y %H:%M:%S')
else:
    basicConfig(
        level=INFO,
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/LavanderProjects/LavanUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/LavanderProjects/LavanUserBot.git" #if not STABILITY else https://github.com/erdewbey/LavanUserBot.git Eski Dostum Berce'ye Aittir.
EMERGENCY = "https://github.com/ber4tbey/LavanUserBot.git" # Acil durrum için
# Afk mesajların iletilmesi
AFKILETME = sb(os.environ.get("AFKILETME", "True"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///lavan.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@LavanUserBot | ")
...
WHITELIST = get('https://raw.githubusercontent.com/LavanderProjects/Lavanubdata/master/whitelist.json').json()
...
URL = 'https://raw.githubusercontent.com/LavanderProjects/LavanUbdata/master/force-surum.check' 
...
URL = 'https://raw.githubusercontent.com/LavanderProjects/Lavanubdata/master/learning-data-root.check'
...
with bot:
    ...
    try:
        bot(JoinChannelRequest("@LavanUserBot"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @LavanUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Owen açabilirsin; Kanala bak` @LavanUserBot')
            else:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@LavanUserBot":
                rev_text = query[::-1]
                ...
                result = builder.article(
                    "@LavanUserBot",
                    text="""@LavanUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/LavanUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/LavanderSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/erdewbey/LavanUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @LavanUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Lavan](https://t.me/LavanUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
