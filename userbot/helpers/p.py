def e_(fayl_adi, name, slep, siyahi):
	f = open(f"./Lavanuserbot{fayl_adi}.py", "x")
...
async def _(misaki):
	for i in a:
		...
		sleep({slep})
Help = CmdHelp("Lavanuserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu plugin @LavanUserBot tarafından hazırlanmıştır.")
Help.add()
								""")
	return f.close()
...
async def misakimusic(misaki):
    ...
    while netice is False:
            ...
            if rast.description == IFACI:
                ...
                await misaki.client.send_file(misaki.chat_id, yukle, caption="@LavanUserBot sizin için `"+rast.description+" - "+rast.title+"` müziğini seçti iyi dinlemeler. :)")
                await event.delete()
                os.remove(yukle)
                netice = True
Help = CmdHelp("Lavanuserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu Plugin @LavanUserBot Tərəfindən Hazırlanmışdır..")
Help.add()
		""".format(
siyahi=siyahi,
name=name,
